...
} from "@/shared/api/feed-content-api";
import { IUpdateFeedItems } from "../model";

export const UpdateContent = ({ updateFeedItemId }: IUpdateFeedItems) => {
  const { data: contentData, isLoading: isFetching } =
    useGetFeedContentQuery(updateFeedItemId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateContent({ id: updateFeedItemId, data: content }).unwrap();
      console.log(content);
      toast.success("Feed Content updated successfully!", {
        description: `Feed Content (ID: ${updateFeedItemId}) has been updated`,
      });
      setShowModal(false);
    } catch (error: unknown) {
